...
from apps.api.messages_api.messages_actions import COACH_CREATOR_IS_CURRENT_USER
...
class CoachAdmin(admin.ModelAdmin):
    ...
    search_fields = ["id",
                     ...
                     "user__id",
                     "thumbnail_link",
                     "coach_speciality__id",
                     "coach_speciality__name",
                     "first_name",
                     "last_name",
                     "phone",
                     "country__id",
    ...
    def get_queryset(self, request):  # Getting queryset once, not each time getting related field, faster
        ...
        #     Q(user__is_staff=True) & Q(user__is_trainer=True))
    ...
    def formfield_for_foreignkey(self, db_field, request, **kwargs):  # Values available to choice
        ...
        if (db_field.name == "coach_creator"
    ...
    def save_model(self, request, obj, form, change):  # Default value for new object
        obj.coach_creator = User.objects.get(id=request.user.id)  # Creator must always be current user
        messages.info(request=request,
                      message=gettext_lazy(COACH_CREATOR_IS_CURRENT_USER))
    ...
    def get_user(self, obj):
        return obj.user
        # return obj.user.objects.filter(is_staff=True, is_trainer=True)
