...
class CountryAllFieldsModelSerializer(serializers.ModelSerializer):
...
class CountryCreateModelSerializer(serializers.ModelSerializer):
    ...
    def validate(self, attrs):
        ...
        if not name_in_attr:
            error_messages.append(COUNTRY_REQUIRED)
            # raise serializers.ValidationError(
            #     gettext_lazy(COUNTRY_REQUIRED))
        ...
        if Country.objects.filter(name=name_in_attr).exists():
            error_messages.append(COUNTRY_EXISTS)
            # raise serializers.ValidationError(
            #     gettext_lazy(COUNTRY_EXISTS))
...
class CountryRetrieveUpdateDeleteModelSerializer(serializers.ModelSerializer):
    creator = serializers.SlugRelatedField(slug_field="id",
                                           read_only=False,
                                           queryset=User.objects.all())
    ...
    def validate(self, attrs):
        ...
        country_id_view_passed_req_param = (
            self.context.get("country_id"))

        old_country_name = Country.objects.get(
            id=country_id_view_passed_req_param).name

        new_country_name = attrs.get("name")

        if new_country_name != old_country_name:
            if Country.objects.filter(
                    name=new_country_name).exists():
                error_messages.append(COUNTRY_EXISTS)
                # raise serializers.ValidationError(
                #     gettext_lazy(COUNTRY_EXISTS))
